...
public class PinchableImageView extends ImageView {
    ...
    private static final int TOUCH_TO_FINISH_DELAY = 200;
     ...
     * @param h       容器的高
     */
    public PinchableImageView(Context context, int w, int h, Handler handler) {
        super(context);
    ...
    public void Rebound(int disX, int disY) {
        this.layout(getLeft() + disX,
                getTop() + disY,
                getLeft() + disX + getWidth(),
                getTop() + disY + getHeight());
    ...
    public boolean onTouchEvent(MotionEvent event) {
        switch (event.getAction() & MotionEvent.ACTION_MASK) {
            ...
            case MotionEvent.ACTION_UP:
                if (timer < TOUCH_TO_FINISH_DELAY && null != mHandler) {
